...
class MallocAllocator final : public cosmic::Allocator {
public:
...
class ThreadPoolRunner final : public cosmic::TaskRunner {
public:
...
public:
    ...
    TestImage(int width, int height, cosmic::PixelDepth depth) { Resize(width, height, depth); }

    void Resize(int width, int height, cosmic::PixelDepth depth) {
        const int bytes_per_pixel = depth == cosmic::PixelDepth::kBits8
                                        ? 4
                                        : (depth == cosmic::PixelDepth::kBits16 ? 8 : 16);
    ...
    cosmic::HostImage& View() { return view_; }
    const cosmic::HostImage& View() const { return view_; }

    void SetPixel(int x, int y, const cosmic::PixelF& value);
    cosmic::PixelF GetPixel(int x, int y) const;

private:
    std::vector<unsigned char> storage_;
    int rowbytes_ = 0;
    cosmic::HostImage view_;
};
