...
void TestImage::SetPixel(int x, int y, const PixelF& value) {
    ...
    switch (view_.depth) {
        case PixelDepth::kBits8: {
            cosmic::Pixel8& p = static_cast<cosmic::Pixel8*>(row)[x];
            p.a = ToByte(value.a);
            p.r = ToByte(value.r);
            p.g = ToByte(value.g);
            p.b = ToByte(value.b);
        ...
        case PixelDepth::kBits16: {
            cosmic::Pixel16& p = static_cast<cosmic::Pixel16*>(row)[x];
            auto to16 = [](float v) {
                return static_cast<std::uint16_t>(std::clamp(v, 0.0f, 1.0f) * cosmic::kMaxChannel16 + 0.5f);
            };
            p.a = to16(value.a);
            p.r = to16(value.r);
            p.g = to16(value.g);
...
PixelF TestImage::GetPixel(int x, int y) const {
    ...
    return cosmic::ReadHostPixel(view_, row, x);
}
...
bool WritePng(const std::string& path, const TestImage& image) {
    const cosmic::HostImage& view = image.View();
