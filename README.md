...
layer alpha ─► content bounds ─► gradient field (type · angle · size · cycles · repeat)
            ─► depth field (dome / sphere / ridge / wave / bulge)
...
* **Depth field** bends the bands into domes, spheres, ridges or waves, or
  magnifies them through a **bulge** (negative depth pinches).
* **Turbulence** adds organic drift. It holds still unless you animate
  *Evolution*; switch on *Loop With Angle* and **one keyframed Angle from 0° to
  360° becomes a seamless loop** of both the gradient and the noise.
...
*Depth shapes: dome, sphere, ridge, wave; bulge, and bulge with negative depth.*
...
text or shape layer. The group just before *Performance* is named after the
build (`v1.1.0 (abc1234)`), so you can confirm which copy is loaded. To update,
...
Projects saved with v1.0 open unchanged: every control keeps its place and its
saved value, and v1.1 only adds to the end of the list.
...
4. For a loop, tick **Turbulence ▸ Loop With Angle** and keyframe **Angle**
...
| Depth | Depth Shape | Dome / Sphere / Ridge / Wave / Bulge | Height map that bends the bands, or (Bulge) a lens that magnifies them. |
| | Depth | −1000 – 1000 % (35 %) | How far the bands are pushed, in palette lengths. Negative makes a bowl. For Bulge, the magnification; negative pinches. |
...
new parameters can only be appended, and popup entries likewise. The match
name is `ABBZ CosmicGradient`. The mock host checks every parameter id against
its index on each run.
...
* `cosmic_tests` — palettes (stops, no overshoot), the seamless loop, bulge,
  defaults, bit-depth agreement, transparency and premultiplication, glow
  energy conservation and bounds, full-frame edges, Full vs Half resolution,
  content bounds, repeat modes, dithering, and memory balance across every
  option combination.
* `cosmic_gpu_tests` — the CUDA kernels compiled as C++ and run by an emulator
  ...
  against the CPU renderer over 27 scenes: they agree to about 1e-6. Emulated
...
* CPU: about 0.15 s for a 1080p full-frame render on 4 cores; defocus no longer
  costs extra on full-frame layers, and about half as much on text.
