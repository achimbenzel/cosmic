#!/usr/bin/env python3
"""Builds the PiPL resource for Cosmic Gradient.
...
Outputs:
  ...
  CosmicPiPLFlags.h   the out-flags, shared with the C++ GlobalSetup
...
# NON_PARAM_VARY is set here and cleared per frame by PF_Cmd_QUERY_DYNAMIC_FLAGS
# unless the grain animates: a dynamic flag must be on in the PiPL to be usable.
OUT_FLAGS = PF_OutFlag_DEEP_COLOR_AWARE | PF_OutFlag_I_EXPAND_BUFFER | PF_OutFlag_NON_PARAM_VARY
...
def write_flags_header(path):
    lines = [
        ...
        "#define COSMIC_OUT_FLAGS %dL" % OUT_FLAGS,
        "#define COSMIC_OUT_FLAGS2 %dL" % OUT_FLAGS_2,
        "",
    ]
    with open(path, "w", encoding="ascii") as handle:
        handle.write("\n".join(lines))
...
def main(argv):
    parser = argparse.ArgumentParser(description="Generate the Cosmic Gradient PiPL resource.")
    parser.add_argument("--sdk-root", required=True, help="After Effects SDK root (contains Headers/)")
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--name", default="Cosmic Gradient")
    parser.add_argument("--match-name", default="ABBZ CosmicGradient")
    parser.add_argument("--category", default="AB Tools")
    parser.add_argument("--support-url", default="")
    parser.add_argument("--basename", default="CosmicGradient")
    ...
    write_flags_header(os.path.join(args.output_dir, "CosmicPiPLFlags.h"))
