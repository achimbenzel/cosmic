...
* point the build at it: `cmake -S . -B build -DAE_SDK_ROOT="C:/AfterEffectsSDK/Examples"`,
...
`dist/CosmicGradient.aex` was built against SDK headers reporting
`PF_PLUG_IN_VERSION 13`, `PF_PLUG_IN_SUBVERS 29` (the After Effects 2023-era
SDK). `tools/generate_pipl.py` reads the spec version out of
`Headers/AE_EffectVers.h`, so the plug-in always advertises the SDK it was
compiled with; newer SDKs work the same way.
